...
from strategy_engineer.board import CrewChiefBackend, HeuristicBackend
from strategy_engineer.memory import RaceMemoryStore
from strategy_engineer import pit_baseline as _pit_baseline
...
# Pickles trained under the old package path still reference this module name.
sys.modules.setdefault("race_engineer.pit_baseline", _pit_baseline)
...
def get_harness_model(model_id: str, replay: RaceReplay, **kwargs: Any) -> HarnessModel:
    ...
    if mid in {"heuristic_sim", "sim"}:
        return SimHarnessModel(
            get_sim_backend("heuristic_sim", replay), model_id="heuristic_sim"
        )
    if mid in {"openai_sim", "openai"}:
        return SimHarnessModel(
            get_sim_backend("openai_sim", replay), model_id="openai_sim"
        )
    if mid == "pit_next_sim":
        return SimHarnessModel(
            get_sim_backend("pit_next_sim", replay), model_id="pit_next_sim"
        )

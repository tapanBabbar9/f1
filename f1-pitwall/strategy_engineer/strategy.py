...
from strategy_engineer.memory import DEFAULT_FINISH_SWING_THRESHOLD, RaceMemoryStore
from shared.racing_rules import DRY_MANDATORY_PIT_RULES
from shared.replay import RaceReplay
from strategy_engineer.sim import OptionCard, oracle_best
from shared.state import RaceState
from shared.tools import OPENAI_TOOL_SCHEMAS, ToolBelt, ToolResult

SIM_SYSTEM_PROMPT = """You are an F1 strategy engineer on the pit wall.
""" + UPCOMING_LAP_DECISION + """
...
- Map pit_next_lap → action pit (upcoming lap); any stay_* option → action stay.
...
- reason = operational one-liner; rationale = fuller strategist note.
...
class SimAgentDecision:
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        if self.radio is not None:
            out["radio_backend"] = self.radio.radio_backend
...
def get_strategy_backend(
    name: str,
    replay: RaceReplay,
    *,
    default_headers: dict[str, str] | None = None,
) -> SimAwareBackend:
    """Strategy-only backend (no radio post-pass)."""
    ...
    if name in {"openai_sim", "openai"}:
        return OpenAISimBackend(replay, default_headers=default_headers)
    if name == "auto":

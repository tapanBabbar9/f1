...
```text
f1-pitwall/
  strategy_engineer/     # phases 1–7 — owns the plan + memory
    pit_baseline.py board.py tools_agent.py
    lap_deg.py sim.py strategy.py memory.py
    crew_chief_eval.py
  race_engineer/         # phase 9 ONLY — radio (read-only on plan)
    radio.py
  shared/                # world model + harness + A2A
    state.py replay.py integrity.py tools.py
    decision.py messages.py pipeline.py
    faithfulness.py racing_rules.py harness.py
  scripts/
...
```bash
python3 -m venv f1-pitwall/.venv
f1-pitwall/.venv/bin/pip install -r f1-pitwall/requirements.txt
```
...
Integrity: `python3 f1-pitwall/scripts/run_integrity.py --samples 2000` → **100%** on 2k samples (2021–2025).
...
```bash
f1-pitwall/.venv/bin/python f1-pitwall/scripts/train_pit_baseline.py   # once, for HGB

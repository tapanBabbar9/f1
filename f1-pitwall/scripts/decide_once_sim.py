...
from strategy_engineer.strategy import replay_race_decisions
...
def main() -> None:
    ...
    parser.add_argument(
        "--radio",
        choices=("heuristic", "openai", "passthrough", "auto"),
        default="heuristic",
        help="Phase 9 Race Engineer radio backend (default: heuristic)",
    )
    parser.add_argument(
        "--dataset",
        type=Path,
    ...
    backend = get_sim_backend(args.backend, replay, radio=args.radio)
    radio_name = getattr(getattr(backend, "radio", None), "name", args.radio)
    print(f"backend: {backend.name}  radio: {radio_name}")
